	...
	<div class="col-lg-12">
		<div class="card">
			<div class="card-header">
				<span class="panel-title">{{ _lang('Update Repaymnet') }}</span>
			</div>
			<div class="card-body">
				@if($ma->status == 1)
					<div class="alert alert-warning">
						<strong>{{ _lang('Payment has already approved.') }}</strong>
					</div>
				@endif
				<form method="post" class="validate" autocomplete="off" action="{{ action('LoanController@update', $id) }}" enctype="multipart/form-data">
					...
					<input name="_method" type="hidden" value="PATCH">
						<h6 class="control-label">{{ _lang('Transaction ID') }} <strong>{{ $ma->txid }}</strong></h6>
					<div class="row">
